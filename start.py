...
def mainEmbedProcess(Size=None, fig='lena.png', Dt=20, rhoT=0, msg="1234"):
	# 基本参数
	Size = Size
	fig = fig
	Dt = Dt
	...
	mesL = len(encode(msg))
	# 读取图片
	# 不知道为什么读取的路径总是有问题，改成下面这个样子了
	img = cv2.imdecode(numpy.fromfile(fig, dtype=numpy.uint8), -1)
	img = img[:Size, :Size]
	...
	if len(border) < 72:
		...
		exit()
	for char, loc in zip(f'{rhoT:016b}' + f'{lastEc:08b}' + f'{La:016b}' + f'{N:032b}',
	...
	cv2.imencode('.png', img)[1].tofile(mfig)
	# 读取嵌入信息的图片
	# mfig = fig
...
def mainExtractProcess(fig='lena.modified.png'):
	...
	mfig = '.'.join(fig.split('.')[:-1] + ['extracted'] + fig.split('.')[-1:])  # lena.modified.png
